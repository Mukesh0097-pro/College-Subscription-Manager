  ...
  return (
    <div className="p-6 lg:p-10 max-w-[1600px] mx-auto space-y-8">
       ...
       <header className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
             <button className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white flex items-center gap-2 shadow-sm">
                <Icon name="add" size={18} /> Add Subscription
             </button>
          </div>
       </header>
       ...
       <section className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {[
             { label: 'Total Subscriptions', val: '24', icon: 'subscriptions', color: 'text-primary', change: '12%', up: true },
             { label: 'Monthly Spend', val: '$342', icon: 'payments', color: 'text-purple-600', change: '5%', up: false },
             { label: 'Savings Rate', val: '18%', icon: 'savings', color: 'text-orange-600', change: '8%', up: true },
          ].map((stat, i) => (
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="col-span-1 lg:col-span-2 rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
             <div className="mb-6 flex justify-between">
                <div><h3 className="font-semibold">Monthly Spending</h3><p className="text-sm text-text-secondary">Current vs Previous</p></div>
                <select className="text-xs border-gray-200 rounded-lg"><option>This Month</option></select>
             </div>
             <div className="relative h-64 w-full flex items-end justify-between gap-4 pl-8">
                ...
                {[45, 55, 65, 85, 60, 35, 25].map((h, i) => (
                   <div key={i} className="relative flex-1 h-full flex flex-col justify-end group">
                      ...
                      <span className="text-center text-xs mt-2 text-text-secondary">{['Jan','Feb','Mar','Apr','May','Jun','Jul'][i]}</span>
                   </div>
                ))}
             </div>
          </div>
          ...
          <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200 flex flex-col">
             ...
             <div className="space-y-6 overflow-y-auto">
                {[
                   { user: 'Netflix', action: "renewal in 3 days", time: '2 min ago', color: 'bg-primary' },
                   { user: 'Alert', action: "Spotify trial ending soon", time: '1 hour ago', color: 'bg-warning' },
                   { user: 'Cancelled', action: "HBO Max subscription", time: '3 hours ago', color: 'bg-gray-300' },
                   { user: 'Added', action: "new: 'Figma Pro'", time: '5 hours ago', color: 'bg-success' }
                ].map((item, i) => (
